...
/**
         ...
         * @package   levantamientorequisitos/application/models/RolGeneral_model.         
         * @version   1.0  Fecha 14-06-2018             
    */
class RolGeneral_model extends CI_Model {
    /**
    ...
    */ 
    function getRole() {
        $this->db->order_by("nombre_rol_negocio", "asc");
        $data = $this->db->get("rol_negocio"); //nombre de la tabla en la base de datos
    ...
    */ 
    ...
    function registrarRol($data) {
        ...
        $this->db->insert('rol_negocio', $data);
    ...
    */ 
    ...
    function rol_Id($id_rol_negocio) {

        $this->db->select('*');
        $this->db->where('id_rol_negocio', $id_rol_negocio);
        $this->db->from('rol_negocio');
        $data = $this->db->get();
        if ($data->num_rows() > 0) {
            return $data->row();
        } else {
    ...
    */ 
    ...
    function actualizarR($data, $id_rol_negocio) {//actualizar
        $this->db->where("id_rol_negocio", $id_rol_negocio);
        $this->db->update('rol_negocio', $data);
        if ($this->db->affected_rows()) {
            return true;
        } else {
            return false;
    ...
    */ 
     ...
     function eliminarRol($id_rol_negocio){
        $this->db->where("id_rol_negocio", $id_rol_negocio);
        $this->db->delete('rol_negocio');
       if ($this->db->affected_rows()) {
            return true;
        } else {            
            return false;
     ...
     * @version   1.0                 
  */ 
    function existe_rol($nombre_rol_negocio) {

        $this->db->select('nombre_rol_negocio');
        $this->db->where("nombre_rol_negocio", $nombre_rol_negocio);
        $this->db->from('rol_negocio');
        $data = $this->db->get();
        if ($data->num_rows() > 0) {
            return true;
